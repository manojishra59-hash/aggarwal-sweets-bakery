...
import { StaffOrder, InventoryItem, ActivityLog } from '../types';
...
export const DashboardTab: React.FC<DashboardTabProps> = ({
  ...
  const uniqueCustomers = Array.from(new Set(safeOrders.map((o) => o.customerPhone || o.customerEmail).filter(Boolean)));
  const totalCustomers = uniqueCustomers.length;
  const lowStockItems = safeInventory.filter((item) => item && Number(item.stockQuantity) <= Number(item.minThreshold));

  const avgOrderValue = safeOrders.length > 0 ? Math.round(todaySales / safeOrders.length) : 0;
    ...
    <div className="space-y-6">
      ...
      <div className="p-6 rounded-2xl bg-gradient-to-r from-[#1A1A1A] via-[#121212] to-[#221B0B] border border-[#D4AF37]/30 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-xl">
        <div>
          ...
          <h2 className="text-2xl font-black text-white font-serif tracking-wide">
            Welcome back, <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#D4AF37] via-[#FFF1A8] to-[#D4AF37]">Staff Portal</span>
          </h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-5 rounded-2xl bg-[#141414] border border-[#D4AF37]/30 hover:border-[#D4AF37] transition-all shadow-lg group">
          ...
          <div className="mt-4">
            ...
            <div className="flex items-center text-emerald-400 text-xs font-bold mt-1 space-x-1">
              ...
              <span>{safeOrders.length > 0 ? 'Live sales' : '0% growth'}</span>
            </div>
          </div>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-[#141414] border border-[#D4AF37]/30 hover:border-[#D4AF37] transition-all shadow-lg group">
          ...
          <div className="mt-4">
            ...
            <div className="flex items-center text-emerald-400 text-xs font-bold mt-1 space-x-1">
              <ArrowUpRight className="w-3.5 h-3.5" />
              <span>{totalCustomers} unique customers</span>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 p-6 rounded-2xl bg-[#141414] border border-[#D4AF37]/30 shadow-xl space-y-4">
          ...
          {/* Golden Bar Graph Chart */}
          <div className="h-48 pt-6 flex items-end justify-between gap-2 border-b border-gray-800 pb-2 px-2">
            {['9 AM', '11 AM', '1 PM', '3 PM', '5 PM', '7 PM', '9 PM'].map((timeSlot, i) => {
              const slotAmount = safeOrders.length > 0 ? Math.round(todaySales / 7) : 0;
              const barHeight = todaySales > 0 ? (slotAmount / todaySales) * 100 : 0;
              return (
                <div key={i} className="flex-1 flex flex-col items-center group relative h-full justify-end">
                  ...
                  />
                  <span className="text-[10px] text-gray-400 mt-2">{timeSlot}</span>
                </div>
              );
            })}
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 pt-2">
            <div className="bg-[#1A1A1A] p-3 rounded-xl border border-gray-800 text-center">
              <span className="text-[10px] uppercase text-gray-400 block">Orders Received</span>
              <span className="text-sm font-bold text-[#F4D03F]">{safeOrders.length}</span>
            </div>
            <div className="bg-[#1A1A1A] p-3 rounded-xl border border-gray-800 text-center">
              <span className="text-[10px] uppercase text-gray-400 block">Avg Order Value</span>
              <span className="text-sm font-bold text-white">₹{avgOrderValue.toLocaleString('en-IN')}</span>
            </div>
            <div className="bg-[#1A1A1A] p-3 rounded-xl border border-gray-800 text-center">
              <span className="text-[10px] uppercase text-gray-400 block">Today's Total</span>
              <span className="text-sm font-bold text-[#F4D03F]">₹{todaySales.toLocaleString('en-IN')}</span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="p-5 rounded-2xl bg-[#141414] border border-[#D4AF37]/30 shadow-xl space-y-3">
            ...
            <div className="space-y-3">
              {safeLogs.length === 0 ? (
                <p className="text-xs text-gray-400 italic">No recent activity logged.</p>
              ) : (
